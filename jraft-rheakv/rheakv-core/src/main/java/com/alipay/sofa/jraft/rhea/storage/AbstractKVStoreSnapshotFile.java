...
public abstract class AbstractKVStoreSnapshotFile implements KVStoreSnapshotFile {
    ...
    @Override
    public void save(final SnapshotWriter writer,
                     final Region region,
                     final Closure done,
        ...
        try {
            doSnapshotSave(snapshotPath, region, executor)
                    .whenComplete((metaBuilder, throwable) -> {
                        ...
                        // snapshot 写入失败
                            ...
                            LOG.error("Fail to save snapshot, path={}, file list={}, {}.",
                                    writerPath, writer.listFiles(), StackTraceUtil.stackTrace(throwable));
                            done.run(new Status(RaftError.EIO,
                                    "Fail to save snapshot at %s, error is %s", writerPath, throwable.getMessage()));
                        }
                    });
        } catch (final Throwable t) {
            LOG.error("Fail to save snapshot, path={}, file list={}, {}.",
                    writerPath, writer.listFiles(), StackTraceUtil.stackTrace(t));
            done.run(new Status(RaftError.EIO,
                    "Fail to save snapshot at %s, error is %s", writerPath, t.getMessage()));
        }
    }
    ...
    abstract CompletableFuture<LocalFileMeta.Builder> doSnapshotSave(
            final String snapshotPath, final Region region, final ExecutorService executor) throws Exception;

    abstract void doSnapshotLoad(
            final String snapshotPath, final LocalFileMeta meta, final Region region) throws Exception;

    protected void compressSnapshot(final SnapshotWriter writer,
        ...
        try {
            ...
            LOG.error("Fail to compress snapshot, path={}, file list={}, {}.",
                    ...
                    "Fail to compress snapshot at %s, error is %s", writerPath, t.getMessage()));
        }
    }
